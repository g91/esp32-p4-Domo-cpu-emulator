...
static struct {
    ...
    // Joystick state (active-low, matching real C64 hardware)
    // joy2 = CIA1 Port A bits 0-4: up=bit0, dn=bit1, lt=bit2, rt=bit3, fire=bit4
    // joy1 = CIA1 Port B bits 0-4: same layout (port 1)
    uint8_t   joy1;    // Joystick port 1 state (0xFF = no buttons)
    uint8_t   joy2;    // Joystick port 2 state (0xFF = no buttons)
...
static uint8_t cia_read(cia_t *cia, uint8_t reg, bool is_cia1) {
    ...
    case 0x00: // Port A - output bits reflect written value, input bits = 0xFF
        if (is_cia1) {
            // CIA1 Port A: joystick port 2 can pull bits 0-4 low (active-low)
            uint8_t base = (cia->port_a & cia->ddr_a) | (~cia->ddr_a & 0xFF);
            return base & (s_emu.joy2 | 0xE0); // bits 0-4 affected by joystick 2
        }
        return (cia->port_a & cia->ddr_a) | (~cia->ddr_a & 0xFF);
    case 0x01: // Port B
        if (is_cia1) {
            // CIA1 Port B: keyboard row input via matrix scanning + joystick port 1
            ...
            return result & (s_emu.joy1 | 0xE0); // joystick port 1 on bits 0-4
        }
...
static void c64_render_to_gpu(void) {
    ...
    bool den  = (d011 & 0x10) != 0; // Display enable (used for logging only)
    (void)den; // We render regardless of DEN - see comment below
    ...
    // Only bits driven as output (ddr_a=1) matter; input bits are pulled high
    // on real hardware, which maps to bank 0 for those bits.
    uint8_t cia2_out = (s_emu.cia2.port_a & s_emu.cia2.ddr_a) | (~s_emu.cia2.ddr_a & 0xFF);
    uint8_t  vic_bank_num = ~cia2_out & 0x03;
    ...
    // Log VIC state once each time a mode change is detected (helps diagnostics)
    static uint8_t s_last_d011 = 0xFF, s_last_d018 = 0xFF, s_last_cia2pa = 0xFF;
    if (d011 != s_last_d011 || d018 != s_last_d018 || cia2_out != s_last_cia2pa) {
        ESP_LOGI(TAG, "VIC state: D011=%02X BMM=%d MCM=%d DEN=%d ECM=%d | "
                      "D018=%02X screen=$%04X bitmap=$%04X char=$%04X | "
                      "CIA2_PA=%02X bank=%d ($%04X-$%04X)",
                 d011, bmm, mcm, den, ecm,
                 d018, screen_addr, bitmap_addr, char_addr,
                 cia2_out, vic_bank_num, vic_bank, (uint16_t)(vic_bank + 0x3FFF));
        s_last_d011   = d011;
        s_last_d018   = d018;
        s_last_cia2pa = cia2_out;
    }
    ...
    // Note: we intentionally do NOT blank the content area when den=0.
    // Many C64 games blank the display briefly each frame (using a raster IRQ
    // to re-enable it). Our 60Hz render tick is not synchronised to the raster
    // IRQ, so if we honoured den=0 here we would permanently black the LCD.
    // The border area is already filled with border_c above (correct), and the
    // content area will render the actual VIC state regardless of DEN.
...
void mos6502_emu_tick(uint32_t cpu_cycles) {
    ...
    if (s_emu.keyboard.release_timer > 0) {
        ...
        if (s_emu.keyboard.release_timer <= 0) {
            ...
            s_emu.joy1 = 0xFF;  // Release all joystick 1 buttons
            s_emu.joy2 = 0xFF;  // Release all joystick 2 buttons
        }
    }
...
static bool c64_load_d64_prg(const char *d64_path, int entry_idx) {
    ...
    // NOTE: must be static to avoid blowing the task stack (64KB > stack size)
...
// ============================================================================
// KERNAL LOAD trap ($FFD5) - intercept LOAD calls to load from mounted D64
// This enables multi-load games that call KERNAL LOAD to fetch additional
// files from disk. Without this, the KERNAL tries to talk to the IEC serial
// bus which we don't emulate, causing the game to hang.
// ============================================================================
static bool c64_kernal_load_trap(void) {
    // Only trap if we have a D64 mounted from the file browser
    if (s_fb_d64_path[0] == '\0') return false;

    // A register: 0 = LOAD, 1 = VERIFY
    if (s_emu.cpu.a != 0) return false;  // Don't intercept VERIFY

    // Read KERNAL workspace variables
    uint8_t fnlen = s_emu.memory[0xB7];          // Filename length
    uint16_t fnadr = s_emu.memory[0xBB] |
                     (s_emu.memory[0xBC] << 8);   // Filename address
    uint8_t fa = s_emu.memory[0xBA];              // Device number
    uint8_t sa = s_emu.memory[0xB9];              // Secondary address

    // Only intercept disk device (8)
    if (fa != 8) return false;

    // Read filename from C64 memory (PETSCII)
    uint8_t petscii_name[17];
    int namelen = (fnlen > 16) ? 16 : fnlen;
    for (int i = 0; i < namelen; i++) {
        petscii_name[i] = s_emu.memory[(fnadr + i) & 0xFFFF];
    }
    petscii_name[namelen] = 0;

    // Open the D64
    FILE *f = fopen(s_fb_d64_path, "rb");
    if (!f) return false;

    // Walk directory to find matching file
    uint8_t dir_track = D64_DIR_TRACK;
    uint8_t dir_sector = D64_DIR_SECTOR;
    uint8_t data_track = 0, data_sector = 0;
    bool found = false;
    ...
    while (dir_track != 0 && !found) {
        ...
        for (int e = 0; e < 8; e++) {
            ...
            // Compare filename (PETSCII, padded with 0xA0)
            bool match = true;
            if (namelen == 1 && petscii_name[0] == '*') {
                // Wildcard: match first PRG file
                match = (ftype == 2);
            } else {
                for (int i = 0; i < namelen; i++) {
                    if (petscii_name[i] == '*') break;  // Wildcard tail
                    if (entry->filename[i] != petscii_name[i]) {
                        match = false;
                        break;
                    }
                }
                // Verify remaining entry chars are padding
                if (match && namelen < 16 && entry->filename[namelen] != 0xA0
                    && entry->filename[namelen] != 0x00) {
                    match = false;
                }
            }
    ...
    if (!found) {
        fclose(f);
        // FILE NOT FOUND - set carry and error code, then RTS
        s_emu.cpu.status |= MOS6502_FLAG_C;
        s_emu.cpu.a = 4;  // FILE NOT FOUND
        // Simulate RTS (pop JSR return address)
        uint8_t lo = s_emu.memory[0x0100 + (uint8_t)(s_emu.cpu.sp + 1)];
        uint8_t hi = s_emu.memory[0x0100 + (uint8_t)(s_emu.cpu.sp + 2)];
        s_emu.cpu.sp += 2;
        s_emu.cpu.pc = (uint16_t)(lo | (hi << 8)) + 1;
        ESP_LOGW(TAG, "KERNAL LOAD: file not found (len=%d)", namelen);
        return true;
    }

    // Read file data following the sector chain
    // Reuse the static buffer from c64_load_d64_prg (safe: never concurrent)
    static uint8_t trap_buf[65536];
    int total = 0;
    uint16_t file_load_addr = 0;
    bool first = true;

    while (data_track != 0 && total < (int)sizeof(trap_buf) - 256) {
        uint32_t offset = d64_ts_to_offset(data_track, data_sector);
        uint8_t sector_data[256];
        fseek(f, offset, SEEK_SET);
        if (fread(sector_data, 1, 256, f) != 256) break;
        ...
        if (first) {
            file_load_addr = sector_data[2] | (sector_data[3] << 8);
            int copy_len;
            if (next_track == 0) {
                copy_len = next_sector - 3;
                if (copy_len < 0) copy_len = 0;
            } else {
                copy_len = 254 - 2;
            }
            memcpy(&trap_buf[total], &sector_data[4], copy_len);
            total += copy_len;
            first = false;
        } else {
            int copy_len;
            if (next_track == 0) {
                copy_len = next_sector - 1;
                if (copy_len < 0) copy_len = 0;
            } else {
                copy_len = 254;
            }
            memcpy(&trap_buf[total], &sector_data[2], copy_len);
            total += copy_len;
        }
    ...
    uint16_t dest_addr;
    if (sa == 0) {
        // SA=0: relocate - use address from X/Y registers
        dest_addr = s_emu.cpu.x | (s_emu.cpu.y << 8);
    } else {
        // SA=1 (or non-zero): load to file's native address
        dest_addr = file_load_addr;
    }

    // Copy into C64 memory
    if (dest_addr + total > 0xFFFF) total = 0xFFFF - dest_addr;
    memcpy(&s_emu.memory[dest_addr], trap_buf, total);

    uint16_t end_addr = dest_addr + total;

    ESP_LOGI(TAG, "KERNAL LOAD trap: -> $%04X-$%04X (%d bytes) SA=%d",
             dest_addr, end_addr, total, sa);

    // Set success return values
    s_emu.cpu.status &= ~MOS6502_FLAG_C;  // Clear carry = success
    s_emu.cpu.a = 0;
    s_emu.cpu.x = end_addr & 0xFF;
    s_emu.cpu.y = (end_addr >> 8) & 0xFF;

    // Update KERNAL end-of-load address ($AE/$AF)
    s_emu.memory[0x00AE] = end_addr & 0xFF;
    s_emu.memory[0x00AF] = (end_addr >> 8) & 0xFF;

    // Simulate RTS (pop JSR return address from stack)
    uint8_t lo = s_emu.memory[0x0100 + (uint8_t)(s_emu.cpu.sp + 1)];
    uint8_t hi = s_emu.memory[0x0100 + (uint8_t)(s_emu.cpu.sp + 2)];
    s_emu.cpu.sp += 2;
    s_emu.cpu.pc = (uint16_t)(lo | (hi << 8)) + 1;
...
static bool c64_file_browser_key(uint8_t key) {
    ...
    case '\n': {
        ...
        // Load ROM file - replace kernal/basic/chargen ROM and reset CPU
        if (has_extension(sel->name, ".rom") || has_extension(sel->name, ".bin")) {
            char fullpath[384];
            snprintf(fullpath, sizeof(fullpath), "%s/%s", s_fb_path, sel->name);
            ...
            if (rf) {
                ...
                if (fsize == C64_KERNAL_ROM_SIZE) {
                    // 8KB ROM - detect kernal vs basic by filename substring
                    char lname[C64_FB_MAX_NAME];
                    strncpy(lname, sel->name, sizeof(lname) - 1);
                    lname[sizeof(lname) - 1] = '\0';
                    for (int ii = 0; lname[ii]; ii++) lname[ii] = (char)tolower((uint8_t)lname[ii]);
                    if (strstr(lname, "basic")) {
                        fread(&s_emu.memory[C64_BASIC_ROM_ADDR], 1, C64_BASIC_ROM_SIZE, rf);
                        ESP_LOGI(TAG, "ROM: loaded BASIC replacement '%s'", sel->name);
                    } else {
                        // Default: treat 8KB ROM as kernal (most common swap case)
                        fread(&s_emu.memory[C64_KERNAL_ROM_ADDR], 1, C64_KERNAL_ROM_SIZE, rf);
                        ESP_LOGI(TAG, "ROM: loaded KERNAL replacement '%s'", sel->name);
                    }
                    fclose(rf);
                    s_fb_active = false;
                    mos6502_reset(&s_emu.cpu);
                    ESP_LOGI(TAG, "CPU reset after ROM swap, PC=$%04X", s_emu.cpu.pc);
                    return false;

                } else if (fsize == C64_CHARGEN_ROM_SIZE) {
                    // 4KB - chargen/character ROM
                    if (s_emu.chargen_rom) {
                        fread(s_emu.chargen_rom, 1, C64_CHARGEN_ROM_SIZE, rf);
                        ESP_LOGI(TAG, "ROM: loaded chargen replacement '%s'", sel->name);
                    }
                    fclose(rf);
                    s_fb_active = false;
                    mos6502_reset(&s_emu.cpu);
                    ESP_LOGI(TAG, "CPU reset after chargen ROM swap, PC=$%04X", s_emu.cpu.pc);
                    return false;

                } else {
                    fclose(rf);
                    ESP_LOGW(TAG, "ROM: unexpected size %ld for '%s' (expected 8192 or 4096)", fsize, sel->name);
                }
            } else {
                ESP_LOGE(TAG, "ROM: cannot open '%s'", fullpath);
            }
            return true;
        }
...
static void c64_poll_keyboard(void) {
    ...
    while (processed < 4) {
        ...
        if (ch < 0x80) {
            ...
            // Also map SPACE to joystick 2 fire button (most games use port 2)
            if (ch == ' ') {
                s_emu.joy2 &= ~0x10;  // FIRE button = bit 4 active-low
            }
            ...
            switch (ch) {
            case PS2_KEY_UP:
                s_emu.joy2 &= ~0x01;  // Joystick 2 UP (bit 0, active-low)
                c64_matrix_press(KS(7,0)); break; // Shift + CRS-DOWN = cursor up
            case PS2_KEY_DOWN:
                s_emu.joy2 &= ~0x02;  // Joystick 2 DOWN (bit 1)
                c64_matrix_press(K(7,0));  break; // CRS-DOWN
            case PS2_KEY_LEFT:
                s_emu.joy2 &= ~0x04;  // Joystick 2 LEFT (bit 2)
                c64_matrix_press(KS(2,0)); break; // Shift + CRS-RIGHT = cursor left
            case PS2_KEY_RIGHT:
                s_emu.joy2 &= ~0x08;  // Joystick 2 RIGHT (bit 3)
                c64_matrix_press(K(2,0));  break; // CRS-RIGHT
...
uint64_t mos6502_emu_run(uint32_t instructions) {
    ...
    if (s_emu.mode == MOS6502_MODE_C64 || s_emu.mode == MOS6502_MODE_C128) {
        // Tick hardware after EACH instruction for accurate raster & timer emulation.
        // Without this, games that poll $D012 (raster counter) or rely on CIA timer
        // IRQs at specific times get stuck because the raster/timers only update
        // after the entire batch of instructions.
        for (uint32_t i = 0; i < instructions && !s_emu.cpu.halted; i++) {
            // KERNAL traps - intercept before execution
            if (s_emu.cpu.pc == 0xFFD5) {
                // $FFD5 = KERNAL LOAD vector
                if (c64_kernal_load_trap()) {
                    s_emu.cpu.cycles += 20; // Approximate cycles for load
                    continue;
                }
            }

            uint64_t cb = s_emu.cpu.cycles;
            mos6502_step(&s_emu.cpu);
            uint32_t elapsed = (uint32_t)(s_emu.cpu.cycles - cb);
            mos6502_emu_tick(elapsed);
        }
    } else {
...
esp_err_t mos6502_emu_init_mode(mos6502_machine_mode_t mode) {
    ...
    case MOS6502_MODE_C64:
        ...
        s_emu.joy1 = 0xFF;  // Joystick port 1: all buttons released
        s_emu.joy2 = 0xFF;  // Joystick port 2: all buttons released
